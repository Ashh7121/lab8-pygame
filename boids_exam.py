...
class Boid:
    ...
    def _screen_wrap(self) -> None:
        ...
        elif self.x < 0: self.x = config.WIDTH
        if self.y > config.HEIGHT: self.y = 0
        elif self.y < 0: self.y = config.HEIGHT
    ...
    def _random_steer(self, spread: float = 0.2) -> None:
        ...
        self.vx = math.cos(angle) * current_speed
        self.vy = math.sin(angle) * current_speed
    ...
    def _alignment(self, boids: List['Boid']) -> pygame.Vector2:
        ...
        for other in boids:
            ...
            if dist < config.ALIGNMENT_DISTANCE:
                avg_vel += pygame.Vector2(other.vx, other.vy)
                total += 1
    ...
    def _cohesion(self, boids: List['Boid']) -> pygame.Vector2:
        ...
        for other in boids:
            ...
            if dist < config.COHESION_DISTANCE:
                avg_pos += pygame.Vector2(other.x, other.y)
                total += 1
        
        if total > 0:
            avg_pos /= total
            steer = avg_pos - pygame.Vector2(self.x, self.y)
